...
## Перевыпуск подписки (2026-09-17, личный кабинет)

`RevokeUserSubscription(uuid)` → `UserResponse` — проксирует
`POST /api/users/{uuid}/actions/revoke` панели с пустым телом: панель
генерирует новый `short_uuid` и новые credentials (vless uuid / trojan / ss),
старая ссылка подписки и старые конфиги перестают работать на всех
устройствах. Срок, статус, сквады и лимиты не меняются. Ответ — обновлённый
пользователь (новые `short_uuid`, `subscription_url`, `sub_revoked_at`).

Это единственное намеренное исключение из правила «не перевыпускать ключи»:
RPC вызывается только по явному подтверждённому действию владельца подписки
в кабинете сайта (там же лимит попыток). Пустой `uuid` — `INVALID_ARGUMENT`,
404 панели — `NOT_FOUND`, остальное — по общему маппингу. В лог пишутся uuid
и новый short_uuid, ссылки — нет.

Аддитивное изменение proto: существующие сообщения и RPC не тронуты, клиенты
без перегенерированных стабов продолжают работать; стабы перегенерированы в
RWMS и на сайте (единственный вызывающий). Вызов старого RWMS новым сайтом
даёт `UNIMPLEMENTED` — кабинет показывает «временно недоступно».
