...
- `INVALID_ARGUMENT` — ставится **явно в хендлере**: невалидный статус,
  стратегия, отрицательный лимит, а также pydantic `ValidationError` при
  сборке DTO **запроса** (например, `email` без точки в домене не проходит
  `EmailStr`). Панели такой запрос не отправлялся, повтор с тем же запросом
  не пройдёт никогда. В `map_exception_to_grpc_code` этой ветки намеренно
  **нет**: SDK валидирует тем же pydantic и **ответы** панели, и общая ветка
  по типу исключения объявляла бы «повтор не поможет» на уже применённую
  мутацию, ответ которой не разобрался;
...
Отдельно про `INVALID_ARGUMENT` (инцидент 2026-09-10): раньше
`ValidationError` попадала в общий `except` и уезжала в `INTERNAL`, то есть
выглядела для клиента как временный сбой панели. Сервис payment ретраит
оплаченные продления бесконечно (задачи конвергентны, хоронить их нельзя),
поэтому продление пользователя с битым `users.email` крутилось в очереди
больше трёх часов: в БД срок продлён, в панели нет. Клиент, различающий
терминальную ошибку, обязан уводить такую задачу в разбор, а не в ретрай.
`ValidationError` ловится именно как `ValidationError`, а не как
`ValueError`-родитель, иначе под тот же код уехали бы посторонние ошибки
вычислений (например из `ProtoTrafficLimitStrategyToRemnawave`).

**Важно:** ни один клиент парка этот код пока не различает — strict-варианты
`RwmsClient.update_user`/`add_user` живут в `common` и отложены до планового
бампа submodule. До тех пор терминальная ошибка по-прежнему уходит в
бесконечный ретрай задачи, поэтому полагаться на `INVALID_ARGUMENT` как на
страховку нельзя.
